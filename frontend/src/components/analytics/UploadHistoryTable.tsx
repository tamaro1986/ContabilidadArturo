  ...
  if (isLoading && history.length === 0) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-24 bg-white rounded-[2rem] animate-pulse border border-zinc-100 shadow-sm" />
        ))}
      </div>
    );
  }
                ...
                <tr key={record.id} className="hover:bg-zinc-50/80 transition-all duration-300 group">
                  ...
                  <td className="px-10 py-7">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-bold text-zinc-600 truncate max-w-[120px]">{record.user_profiles?.full_name || 'Desconocido'}</span>
                    </div>
                  </td>
                  <td className="px-10 py-7">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-zinc-900">{record.records_processed.toLocaleString()}</span>
                      <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-[0.1em] mt-0.5">Registros</span>
                    </div>
                  </td>
                  <td className="px-10 py-7">
