        ...
        <div className={`fixed inset-0 z-[200] flex justify-end transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            <div className={`relative w-full max-w-lg bg-white h-full shadow-2xl transition-transform duration-500 ease-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                ...
                <div className="p-8 border-b border-zinc-100 flex items-center justify-between">
                    <div>
                        <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-1">Detalle del Cliente</p>
                        <h2 className="text-xl font-black text-zinc-900 tracking-tight">{customer?.nombre}</h2>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-10">
                    ...
                    <div className="relative">
                        <div className="bg-primary/5 border border-primary/10 rounded-2xl p-6 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-2 text-[10px] font-black text-primary uppercase tracking-widest mb-4 relative z-10">
                                <SparklesIcon />
                                Recomendación de IA
                            </div>
                                ...
                                <div key={i} className="flex items-center justify-between p-4 border border-zinc-100 rounded-xl hover:border-zinc-200 transition-colors">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-2 h-2 rounded-full ${
                                            inv.estado === 'Pagada' ? 'bg-emerald-500' : 
                                            inv.estado === 'Pendiente' ? 'bg-amber-500' : 'bg-red-500'
                                        }`} />
                                        <div>
                                            <p className="text-xs font-bold text-zinc-900">{inv.concepto}</p>
                                            <p className="text-[10px] font-medium text-zinc-400 mt-0.5">{inv.fecha}</p>
                                        </div>
                                    </div>
                ...
                <div className="p-8 border-t border-zinc-100 bg-zinc-50/30">
                    <button className="w-full py-4 bg-primary text-on-primary rounded-xl text-xs font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-primary/20">
