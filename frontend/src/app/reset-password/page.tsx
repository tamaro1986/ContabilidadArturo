...
const Check = ({ size = 18, className = "" }: { size?: number; className?: string }) => (
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    // Verificar si hay una sesión activa (Supabase debería haberla cargado desde el fragmento de la URL)
    const checkSession = async () => {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) {
        // Si no hay sesión, tal vez el enlace expiró o no es válido
        // Intentamos esperar un poco por si el SDK está procesando el fragmento
        setTimeout(async () => {
          const { data: { session: retrySession } } = await supabase.auth.getSession()
          if (!retrySession) {
            setError('El enlace de recuperación ha expirado o no es válido. Por favor, solicite uno nuevo.')
            setVerifying(false)
          } else {
            setVerifying(false)
          }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true)
      // Cerrar sesión después de cambiar clave para forzar nuevo login
    ...
    <div className="min-h-screen flex items-center justify-center bg-surface relative overflow-hidden">
      ...
      <div className="max-w-md w-full mx-4 relative z-10">
        ...
        <div className="bg-surface-container-lowest p-10 rounded-2xl border border-outline-variant shadow-2xl backdrop-blur-sm bg-opacity-80">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-primary mb-2">Restablecer Clave</h2>
            <p className="text-on-surface-variant text-sm font-medium">Defina su nueva credencial de acceso de alta seguridad.</p>
          </div>
          ...
          {success ? (
            <div className="text-center space-y-4">
              ...
              <h3 className="text-xl font-bold text-emerald-800">¡Contraseña Actualizada!</h3>
              <p className="text-on-surface-variant text-sm">Su clave ha sido cambiada. Redirigiendo al ingreso en unos segundos...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest ml-1">Nueva Clave</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest ml-1">Confirmar Clave</label>
                <div className="relative group">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-outline group-focus-within:text-primary transition-colors">
                    <Lock size={18} />
                  </div>
              ...
              >
                {loading ? (
                  ...
                  <span className="text-sm font-black uppercase tracking-[0.2em]">Actualizar Clave</span>
                )}
              </button>
            </form>
          )}
